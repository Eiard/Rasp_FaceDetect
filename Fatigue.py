...
def inmouth_aspect_ratio(inmouth):  # 内部嘴部

    A = np.linalg.norm(inmouth[1] - inmouth[7])  # 62, 68
    B = np.linalg.norm(inmouth[2] - inmouth[6])  # 63, 67
    C = np.linalg.norm(inmouth[3] - inmouth[5])  # 64, 66
    D = np.linalg.norm(inmouth[0] - inmouth[4])  # 61, 65
...
try:
    _thread.start_new_thread(CountTime, (1,))
except:
    print("Error: 无法启动线程")
...
while True:
    ...
    for rect in rects:
        ...
        inmar = inmouth_aspect_ratio(inmouth)
        ...
        # 同理，判断是否打哈欠
        if mar > MAR_THRESH:  # 张嘴阈值
            if inmar > INMAR_THRESH:  # 内部张嘴阈值
                mCOUNTER += 1
        else:
            # 如果连续2次都小于阈值，则表示打了一次哈欠
            if mCOUNTER >= MOUTH_AR_CONSEC_FRAMES:  # 阈值：
                mTOTAL += 1
            # 重置嘴帧计数器
            mCOUNTER = 0
        ...
        print('内嘴巴实时长宽比:{:.2f} '.format(inmar) + "\t是否张嘴：" + str([False, True][inmar > MAR_THRESH]))
